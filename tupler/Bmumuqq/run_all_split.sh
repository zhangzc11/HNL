...
nFilesPerJob=$1
data_or_mc=$2
LFNs_file=LFNs_42912016_20GeVCtau0ps.txt
options=options_Bd.py
...
if [[ "$data_or_mc" == "MC" ]] || [[ "$data_or_mc" == "mc" ]]; then
    ...
    #prepare hadd script
    echo "hadd -k -f dv_ntuple_mc_$work_dir.root dv_ntuple_mc_*.root" >> cmd_hadd.sh
...
if [[ "$data_or_mc" == "data" ]]; then
    ...
    #prepare hadd script
    echo "hadd -k -f dv_ntuple_data_$work_dir.root dv_ntuple_data_*.root" >> cmd_hadd.sh
    chmod 744 cmd_hadd.sh
fi
