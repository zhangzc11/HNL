...
def diyline():
    ...
    combination_code_Z0 = F.require_all(
        F.MASS > 60. * GeV,
        F.MASS < 120. * GeV,
    )

    vertex_code_Z0 = F.require_all(
        F.MASS > 60. * GeV,
        F.MASS < 120. * GeV,
    )
    ...
    Z0 = ParticleCombiner(
        name="myZmumu",
        Inputs=[muons, muons],
        DecayDescriptor=decay_Z0,
        CombinationCut=combination_code_Z0,
        CompositeCut=vertex_code_Z0)

    return Z0
  
def make_algs_Zmumu(options, algname):
    data = diyline()
    #data = get_particles(f"/Event/Spruce/SpruceQEE_ZToMuMu/Particles")
    ...
    variables = {
        #"ALL": all_vars,
        "Z": all_vars+vertex_vars+mc_truth_vars,
        "Mum": all_vars+track_vars+mc_truth_vars+utils.make_isolation_vars(data, "Mum", "Z"),
        "Mup": all_vars+track_vars+mc_truth_vars+utils.make_isolation_vars(data, "Mup", "Z"),
    }
...
def main(options: Options):
    ...
    algs = make_algs_Zmumu(options, name)
