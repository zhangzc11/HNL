...
def get_pt_theoretical_distribution(m_N, bins=100, pt_max=60):
    """利用四维张量网格进行平滑的理论边缘积分投影 (针对 mu_N)"""
    N_grid = 50
...
def main():
    ...
    print("[*] 正在计算理论四重积分生成 mu_N 横动量验证图，请稍候(约5秒)...")
    
    for m, c in zip(masses, colors):
        file_pattern = f"{data_dir}/*_{m}GeVCtau0ps_*.root"
        
        # 1. 读取 MC
        mc_pt_mu = get_mc_data(file_pattern, "MuNuR_PT")
        if len(mc_pt_mu) > 0:
            plt.hist(mc_pt_mu, bins=60, range=(0, 60), density=True, 
                     histtype='step', color=c, lw=2.0)
            
        # 2. 纯理论计算
        b_mu, d_mu = get_pt_theoretical_distribution(m, pt_max=60)
        plt.plot(b_mu, d_mu, color=c, lw=2.5, linestyle='--', label=f'$m_N={m}$ GeV')

    plt.title(r"Validation: Secondary Muon $P_T$ ($P_{T, \mu_N}$)", fontsize=16)
    plt.xlabel(r"$P_{T, \mu_N}$ [GeV]", fontsize=14)
    ...
    plt.tight_layout()
    plt.savefig("Validation_PT_muN.png", dpi=300)
    print("[+] 保存成功：Validation_PT_muN.png")
