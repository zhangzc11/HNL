...
MC_FILES = sorted(glob.glob(os.path.join(DATA_DIR, "42912010_5GeVCtau0ps_options_20260228_111712_skim.root")))
...
VARIABLES = [
    "W_MINIPCHI2",                            # 几何
    "MuNuR_MINIP",                            # 几何
    "NuR_MAX_SDOCACHI2",                      # 几何
    #"NuR_ALV",                                # 几何(有严重的堆积?)
    "Mu_PT",                                  # 动力学
    "NuR_MIN_PT",                             # 动力学
    "W_PT",                                   # 动力学
    "W_ETA",                                  # 角度 (替代了 THETA)
    "Mu_PROBNN_MU",                           # 重建为谬子概率过于尖锐，建议审慎考虑降低风险
    "Mu_PROBNN_GHOST",                        # 径迹质量
    "Mu_PROBNN_PI",                           # PID
    "Mu_HCALEOP",                             # PID
    #"Mu_POSITION_STATEAT_LastMeasurement_Z",  # 空间位置
    "nTTracks",                               # 事件级
    "Mu_BREMTRACKBASEDENERGY",                # 能量修正
    "MuNuR_E"                                 # 能量 (去除了高度相关的 log_MIN_P)
]
