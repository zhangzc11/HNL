...
def draw_and_save(mc_path, data_path, tree_path, branch_name):
    ...
    # 创建直方图
    h_mc = TH1F("h_mc", f";{branch_name};Normalized Yields", 100, global_min, global_max)
    h_data = TH1F("h_data", f";{branch_name};Normalized Yields", 100, global_min, global_max)

    tree_mc.Draw(f"{branch_name} >> h_mc", "", "goff")
    tree_data.Draw(f"{branch_name} >> h_data", "", "goff")
